    ...
    try {
      ...
      Object.entries(activities).forEach(([name, details]) => {
        ...
        if (details.participants && details.participants.length) {
          participantsHTML = `<div class="participants"><strong>Participants:</strong><ul>${details.participants.map(p => `<li>${p} <button class="remove-btn" data-activity="${encodeURIComponent(name)}" data-email="${p}">✖</button></li>`).join('')}</ul></div>`;
        } else {
